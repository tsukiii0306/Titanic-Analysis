...
def get_data():
    ...
    df_family_status = df.groupby("ticket").agg(
        family_counts=("ticket", "count"), children_count=("children", "sum"), fare_price=("fare", max)
    )
    df_family_status["adult_count"] = df_family_status.family_counts - df_family_status.children_count
    df_family_status["adult_price"] = (
        2 * df_family_status.fare_price / (df_family_status.children_count + 2 * df_family_status.adult_count)
    )
    df_family_status["children_price"] = df_family_status.fare_price / (
        df_family_status.children_count + 2 * df_family_status.adult_count
    )
    dic_adult_price = df_family_status.adult_price.to_dict()
    dic_children_price = df_family_status.children_price.to_dict()
    df["fare_new"] = df.apply(
        lambda x: dic_children_price[x["ticket"]] if x["children"] == 1 else dic_adult_price[x["ticket"]], axis=1
    )
